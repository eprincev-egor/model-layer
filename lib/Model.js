...
class Model extends EventEmitter {
    ...
    toJSON() {
        ...
        for (let key in this.data) {
            let value = this.data[ key ];
            let description = this.structure[ key ] || anyKeyDescription;
...
Model.prepareObject = function(description, modelKey, originalObject) {
    ...
    for (let key in originalObject) {
        ...
        try {
            element = prepareValue( elementDescription, key, element );
        } catch(err) {
            let valueAsString = invalidValuesAsString( originalObject );
...
function normalizeDescription(description) {
    ...
    // user: {type: {}}
    if ( description.type && typeof description.type == "object" ) {
        let elementDescription = description.type.element || "*";
        elementDescription = normalizeDescription( elementDescription );
    ...
    if ( !("type" in description) ) {
        ...
        if ( "element" in description ) {
            description.element = normalizeDescription( description.element );
        }
        // user: {}
        else {
            description.element = {
