...
class Collection extends EventEmitter {
    ...
    // https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/Array/copyWithin
    copyWithin(target, start, end) {
        // stepи 3-5.
        let len = this.models.length >>> 0;
        ...
        // step 18
        while (count > 0) {
            if (from in this.models) {
                let model = this.models[from];
                let clone = model.clone();

                this.models[to] = clone;
            } else {
                delete this.models[to];
            }
